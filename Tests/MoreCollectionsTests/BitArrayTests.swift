...
final class BitArrayTests: XCTestCase {
  ...
  func testInverse() {
    let a0 = BitArray((0 ..< 100).map({ $0 % 2 == 0 }))
    XCTAssertEqual(~a0, BitArray((0 ..< 100).map({ $0 % 2 != 0 })))
  }
  ...
  func testHashable() {
    XCTAssertEqual(BitArray().hashValue, BitArray().hashValue)
    let a0 = BitArray((0 ..< 100).map({ $0 % 2 == 0 }))
    var a1 = a0
    a1.reserveCapacity(1000)
  ...
  func testMutableCollection() {
    var a0 = BitArray((0 ..< 100).map({ $0 % 2 == 0 }))
    for i in 0 ..< 100 {
      a0[i] = !a0[i]
    }
    XCTAssertEqual(a0, BitArray((0 ..< 100).map({ $0 % 2 != 0 })))
  ...
  func testPositionComparable() {
    let a0 = BitArray((0 ..< 100).map({ $0 % 2 == 0 }))
    let p0 = a0.startIndex
    XCTAssertLessThan(p0, a0.index(after: p0))
    XCTAssertLessThan(p0, a0.endIndex)
  }
