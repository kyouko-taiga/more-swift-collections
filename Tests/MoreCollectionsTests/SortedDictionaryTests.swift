...
class SortedDictionaryTests: XCTestCase {
  ...
  func testInitWithMinimumCapacity() {
    let s = SortedDictionary<String, Int>(minimumCapacity: 100)
    XCTAssertGreaterThanOrEqual(s.capacity, 100)
  }

  func testIsEmpty() {
    var s = SortedDictionary<String, Int>()
    XCTAssert(s.isEmpty)
    s["a"] = 1
  ...
  func testIndexForKey() {
    ...
    XCTAssertEqual(s.index(forKey: "c"), 1)
  }
  ...
  func testReinsertPair() {
    var s: SortedDictionary = ["a": 1, "b": 2, "c": 3]
    s["b"] = nil
    s["b"] = 200
    XCTAssertEqual(s.index(forKey: "b"), 1)
  }
  ...
  func testRemoveAt() {
    ...
    XCTAssertEqual(s.remove(at: 1).key, "c")
  ...
  func testIsEquatable() {
    let s: SortedDictionary = ["a": 1, "b": 2]
    let t: SortedDictionary = ["b": 2, "a": 1]
    XCTAssertEqual(s, s)
    XCTAssertEqual(s, t)
    let u: SortedDictionary = ["b": 2, "c": 1]
    XCTAssertNotEqual(s, u)
  }

  func testIsHashable() {
    let s: SortedDictionary = ["a": 1, "b": 2]
    let t: SortedDictionary = ["a": 1, "b": 2]
    XCTAssertEqual(s.hashValue, t.hashValue)
  }

  func testIsCustomStringConvertible() {
    let s: SortedDictionary = [1: -1, 2: -2]
